...
describe ConferencesController do
  ...
  before :each do
    allow_any_instance_of(BigBlueButtonConference).to receive(:send_request).and_return('')
    allow_any_instance_of(BigBlueButtonConference).to receive(:get_auth_token).and_return('abc123')
  end
  ...
  describe "GET 'recording'" do
    it "should require authorization" do
      ...
      get 'recording', params: {:course_id => @course.id, :conference_id => @conference.id,
  ...
  describe "DELETE 'recording'" do
    it "should require authorization" do
      @conference = @course.web_conferences.create!(:conference_type => 'BigBlueButton', :duration => 60, :user => @teacher)
      delete 'recording', params: {:course_id => @course.id, :conference_id => @conference.id,
        :recording_id => 'abc123-xyz'
      }
      assert_unauthorized
    end
